...
@Component({
  selector: 'se-user-menu',
  templateUrl: './se-user-menu.component.html'
})
export class SeUserMenuComponent implements OnInit, OnDestroy {
  ...
  updateMenu() {
    ...
    this.menu = new Action({
        icon: this.icon,
        text: this.principalService.principal ? this.userDisplay(this.principalService.principal) || "msg#userMenu.user" : "msg#userMenu.user",
        children: userActions
    });
  }

  userDisplay(p: Principal):string {
    return `${p.param9} ${p.param8} (${p.param7})`;
  }
